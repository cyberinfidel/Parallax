...
def director_meet(game):
	...
	return [
		...
		Spawn(spawns=[SpawnEntity('pinkie meet','pinkie', Vec3(220,150,1), game)]),
		Delay(1.6),
		Message("Blue.", Vec3(240,180,0), bunnies['blue'].color, 2, px_graphics.eAlign.centre),
		Spawn(spawns=[SpawnEntity('blue meet','blue', Vec3(240,150,1), game)]),
		Delay(0.9),
		Message("Bowie.", Vec3(260,180,0), bunnies['bowie'].color, 2, px_graphics.eAlign.centre),
		Spawn(spawns=[SpawnEntity('bowie meet','bowie', Vec3(260,150,1), game)]),
		Delay(1.5),
		Message("and of course:", Vec3(240,130,0), Color(1, 1, 1), 2.5, px_graphics.eAlign.centre),
		Delay(2),
		Message("PacBun", Vec3(240,120,0), bunnies['pacbun'].color, 3, px_graphics.eAlign.centre),
		Spawn(spawns=[SpawnEntity('pacbun meet','pacbun', Vec3(240,85,1), game)]),
		Delay(3),
		SpawnDirector('fade_to_next_scene')
	]
...
def director_high_scores(game):
	return [
		FadeToClearColor(Color(0, 0, 0), 2),
		FadeRenderLayer('overlay', Color(1,1,1,1),1),
		Message("Best", Vec3(212, 287, 0), Color(1, 1, 0, 1), -1, px_graphics.eAlign.left),
		...
		Delay(0.8),
		Message("est", Vec3(235, 287, 0), Color(0, 1, 1, 1), -1, px_graphics.eAlign.left),
		Delay(0.8),
		Message("Buns", Vec3(255, 287, 0), Color(1, 0, 1, 1), -1, px_graphics.eAlign.left),
		Delay(3),
		# FadeRenderLayer('overlay', Color(0,0,0,0),1),
		# Delay(1),
		SpawnDirector('fade_to_next_scene')
...
def director_play(game, data):
	...
	events.extend([
		...
		Delay(1),
		# Spawn(spawns=[SpawnEntity('director', init="import PB_directors\n"
